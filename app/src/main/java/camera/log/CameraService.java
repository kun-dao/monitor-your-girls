...
public class CameraService extends Service {
    ...
    public void onCreate() {
        super.onCreate();
        Log.v("CameraService", "CameraService  onCreate");
        // 定时器类
        Timer timer = new Timer();
        timer.schedule(task, 1000, 1000 ); // 1s后执行task,经过1min再次执行
...
//        }
    ...
    private void  removeFaceTrackingView() {
        if (myWindow != null) {
    ...
    public void onDestroy() {
        super.onDestroy();
        Log.v("CameraService", "CameraService  onDestroy");
        mWindowManager.removeViewImmediate(myWindow);
        myWindow = null;
    }
}